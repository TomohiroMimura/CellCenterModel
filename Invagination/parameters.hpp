...
constexpr char OFF_FILE_NAME[] = "InitSquare_periodic20.off";//入力offファイルの名前
constexpr double DELTA_TIME = 2e-4;//時間間隔
constexpr int STEP_END=100000;//繰り返し回数
...
constexpr double K_APICAL_DIS_S=10;//apical収縮の収縮力(surface)
constexpr double K_APICAL_DIS_L=10;//apical収縮の収縮力(line)
...
constexpr int SurfaceApicalConstriction_circleMode=1;//1:あり
...
constexpr int CellDivisionRegionMode=3;//1:全体で分裂,2:外側で分裂,3:内側で分裂,0:全体でも分裂しない
...
//周期領域の計算領域(xy平面)
constexpr double size_x=20;
constexpr double size_y=20*sqrt(3)/2.0;

constexpr int BOUNDARY_FIX_MODE=1;//1:境界点を固定,0:境界点を固定しない(when non-periodic mode)
constexpr int CELL_DIVISION_MODE=1;//0:なし,1:ランダム方向分裂軸,2:最長方向分裂軸,3:x方向分裂軸,4:円周方向分裂軸
