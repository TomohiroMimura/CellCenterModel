...
constexpr char OFF_FILE_NAME[] = "InitSquare3.off";//入力offファイルの名前
constexpr double DELTA_TIME = 2e-4;//時間間隔
constexpr int STEP_END=800000;//繰り返し回数
constexpr int OUTPUT_VTK_STEP=1000;//何回おきにvtkファイルを出力するか
constexpr int CELL_DIVISION_STEP_AVE=100000;//何回おきに分裂するか(平均)
constexpr int CELL_DIVISION_STEP_SD=1000;//細胞周期の標準偏差

constexpr double alpha=0.99999;
constexpr int TurnOffStep=300000;
...
//cell_time=DELTA_TIME*CELL_DIVISION_STEP_AVE*(1+DIV_alpha*std::pow(r_dis,DIV_beta))を用いる
...
constexpr double K_Z=0;//z方向の変形を抑制(-/+)0.0000625,0.000125,0.00025,0.001,0.002,0.004,0.008,0.016
...
constexpr double FLIP_THRESHOLD_LENGTH=0.43;//flip判定の閾値(基本的には0.5773)
constexpr int FLIP_COOLTIME_STEP=5;//一度flipした後のcool_timeの長さ

constexpr int PERIODIC_MODE=0;//periodic:1,non-periodic:0,Y-axis periodic:2
//周期領域の計算領域(xy平面)
constexpr double size_x=40;
constexpr double size_y=40*sqrt(3)/2.0;

constexpr int BOUNDARY_FIX_MODE=0;//1:境界点を固定,0:境界点を固定しない(when non-periodic mode)
constexpr int CELL_DIVISION_MODE=0;//0:なし,1:ランダム方向分裂軸,2:最長方向分裂軸,3:x方向分裂軸,4:円周方向分裂軸
